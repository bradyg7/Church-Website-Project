...
(function(){
    ...
    /**
     * Initializes click event listeners for all sermon cards.
     * When a sermon card is clicked, it triggers the handleSermonCardClick function.
     */
    ...
    /**
     * Handles the click event on a sermon card.
     * Extracts sermon details from data attributes and navigates to either
     * 'player.html' (for direct video src) or 'sermon.html' (for YouTube embeds).
     * @param {HTMLElement} card - The sermon card HTML element that was clicked.
     */
    ...
    async function loadEvents() {
        const path = window.location.pathname;
        if (!eventsList || !path.endsWith('all-events.html')) return;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Load events only if on the all-events.html page
        if (window.location.pathname.endsWith('all-events.html')) {
            loadEvents();
        }
